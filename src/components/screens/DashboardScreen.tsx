...
export function DashboardScreen() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center" style={{ background: "#F5F6FA" }}>
        <form
          ...
          style={{ borderTop: "6px solid #F57C00" }}
        >
          <h2 className="mb-1.5 text-[22px] font-bold" style={{ color: "#1A3C5E" }}>
            Coordinator Sign In
          </h2>
          <p className="mb-5 text-sm text-gray-500">Access restricted to authorized barangay coordinators.</p>

          <label className="mb-1.5 block text-[13px] font-semibold" style={{ color: "#1A3C5E" }}>
            Email
          </label>
          <input
            type="email"
  ...
  return (
    <PageLayout>
      <div className="flex h-screen flex-col overflow-hidden" style={{ background: "#F5F6FA" }}>
        <div className="flex flex-1 overflow-hidden">
          {/* Sidebar */}
          <aside className="hidden h-full w-64 shrink-0 flex-col border-r border-gray-200 py-8 md:flex" style={{ background: "#F0F2F5" }}>
            <div className="mb-8 flex items-center gap-3 px-6">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full" style={{ background: "#1A3C5E" }}>
                <svg className="h-6 w-6 text-white" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
                </svg>
              </div>
              <div>
                <h2 className="text-sm font-bold text-gray-900 line-clamp-1">Barangay Coordinator</h2>
                <p className="text-xs text-gray-500">District IV HQ</p>
              </div>
            </div>
              ...
              {SIDEBAR_ITEMS.map((item) => (
                <button
                  ...
                  className={`flex items-center gap-3 rounded-r-xl border-l-4 px-3 py-2.5 text-sm transition-all ${
                    item.active
                      ? "border-amber-500 font-bold text-gray-900"
                      : "border-transparent text-gray-500 hover:bg-gray-200/50"
                  }`}
                  style={item.active ? { background: "rgba(26,60,94,0.05)" } : {}}
                >
            ...
            <div className="mt-auto flex flex-col gap-4 px-6 pb-4">
              ...
              <div className="flex flex-col gap-1 border-t border-gray-200 pt-4">
                <span className="flex items-center gap-3 py-2 text-sm text-gray-500 transition-colors hover:text-gray-900 cursor-pointer">
                  <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
          ...
          {/* Main Content */}
          <main className="flex flex-1 flex-col overflow-hidden" style={{ background: "#F5F6FA" }}>
            <div className="flex-1 overflow-y-auto p-4 md:p-8">
              <div className="mx-auto flex max-w-7xl flex-col gap-8">
                ...
                <section className="grid grid-cols-2 gap-6 md:grid-cols-4">
                  <div className="flex flex-col gap-3 rounded-xl bg-white p-6 shadow-sm" style={{ borderTop: "4px solid #1A3C5E" }}>
                    <div className="flex items-center gap-2" style={{ color: "#1A3C5E" }}>
                      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm0 12H4V8h16v10z" />
                      </svg>
                      <span className="text-sm font-bold uppercase tracking-wider">Total Reports</span>
                    </div>
                    <div className="text-[42px] font-bold leading-none" style={{ color: "#1A3C5E" }}>
                      124
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 rounded-xl bg-white p-6 shadow-sm" style={{ borderTop: "4px solid #F57C00" }}>
                    <div className="flex items-center gap-2" style={{ color: "#F57C00" }}>
                      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                ...
                {/* Alert Banner */}
                <div className="flex items-center justify-between gap-4 rounded-xl p-4 shadow-sm text-white" style={{ background: "#F57C00" }}>
                ...
                <section className="flex flex-col gap-4">
                  <h2 className="text-xl font-bold" style={{ color: "#1A3C5E" }}>Recent Submissions</h2>
